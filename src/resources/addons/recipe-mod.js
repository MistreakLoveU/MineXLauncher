...
function displayMessage(msg) {
    ...
    if (msg === "success") {
        ...
        document.querySelector(':root').style.setProperty('--hue-rotate',
 "0deg");
...
function craft(result,
 index) {
    ...
    let craft = getRecipe(getRecipesForId(ModAPI.getIdforItem(result)),
 index);
    ...
    craft.grid.forEach(item => {
        ...
        if (item) {
            ...
            matrixGrid.push(ModAPI.reflect.classes[0].constructors[4].exec({
itemIn: ModAPI.items[item].getRef(),
 amount: 1
}));
    ...
    resultItem = ModAPI.reflect.classes[0].constructors[4].exec({
itemIn: ModAPI.items[craft.result].getRef(),
 amount: craft.resultNumber
})
...
function currentScreenName() {
    ...
    return String(Minecraft.$currentScreen).substring(String(Minecraft.$currentScreen).lastIndexOf(".") + 1,
 String(Minecraft.$currentScreen).lastIndexOf("@"));
...
url
 ...
 url('https://raw.githubusercontent.com/kuronekony4n/mcicons/main/assets/itemBg.png')`;

    item.style.backgroundSize = "75% 75%,
 cover";
...
function addAllItems() {
    ...
    Object.keys(recipes).forEach(item => {
        ...
        if (exceptions[name]) {
            ...
            url = `https://raw.githubusercontent.com/kuronekony4n/mcicons/main/icons-1.21-thumb/${
exceptions[name]
}.png`;
...
ModAPI.addEventListener("update",
 () => {
    ...
    if (window.recipeGui) {
        ...
        textInput.style.textShadow = `${
ModAPI.ScaledResolution.getScaleFactor()
}px ${
ModAPI.ScaledResolution.getScaleFactor()
}px 0px rgb(55 55 55 / 75%)`;


        itemList.style.height = `calc(100% - ${
ScaledResolution * 22
}px)`;
        ...
        document.querySelector(':root').style.setProperty('--itemSize',
 (ModAPI.ScaledResolution.getScaleFactor() * 17.25) + "px");

        document.querySelector(':root').style.setProperty('--sb-size',
 (ModAPI.ScaledResolution.getScaleFactor() * 5) + "px");
...
document.addEventListener('keydown',
 function(event) {
    ...
    if (window.recipeGui && event.key.length == 1 && window.searchFieldValue.length < 30) {
        ...
        window.searchFieldValue = window.searchFieldValue.slice(0,
-1);
...
ModAPI.addEventListener("frame",
 () => {
    ...
    document.querySelector(':root').style.setProperty('--scale',
 "scale("+ ModAPI.ScaledResolution.getScaleFactor()/3 +")");
